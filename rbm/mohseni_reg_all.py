...
temp_d = pickle.load( open( "basin_temps_all.p", "rb"))
huc_ix = pickle.load( open( "huc_ix.p", "rb" ) )
...
#		self.basin = basin
	...
	def make_loc_d(self, basin):
		f = pd.DataFrame.from_dict(huc_ix[basin], orient='index')
		f['ilat'] = [ast.literal_eval(o) for o in f['lat']]
		f['ilon'] = [ast.literal_eval(b) for b in f['lon']]
		f['latlon'] = zip(f['ilat'], f['ilon'])
		self.loc_d.update({basin : f[['latlon', 'nm']]})
	...
	def relate_met(self, basin, **kwargs):
		...
		if len(temp_d[basin]) > 0:
			...
			fn_d = {}
			ll = self.loc_d[basin]['latlon'][st_id]
#			print ll
	...
	def plot_curvefit(self, basin, **kwargs):
		...
		if self.isavg[st_id] == True:
			...
			if len(ydata) < 1:
				...
				statname = self.loc_d[basin]['nm'][st_id]
...
m = make_mohseni()
#for q in temp_d['california'].keys():
m.prep_data('california', automax=False, st_id='11046072')
m.reg_exec('california', automax=False, st_id='11046072', plot_results=True)
