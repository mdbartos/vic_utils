...
class rbm_post():
	...
	def __init__(self, op_path, rc_path, rbm_path, atm_path):
...
#				with open('%s/%s/fluxes_%s_%s' % (flow_path, scen, self.lld[pcode][0], self.ll_d[pcode][1]) as flowfile:
#					line_d.update('flow' : flowfile.readlines())
#				for lin in line_d['wtemp']:
#					print lin
				...
				print wtemp_df #groupby('DATE', sort=True).mean()
				...
				print atmo_df
				cat_df = pd.concat([wtemp_df, atmo_df], join='inner', axis=1)
				print cat_df
#				outfile.write(str(line_d))
				
#	def make_st_outfiles(self, scen, basin, wpath):
